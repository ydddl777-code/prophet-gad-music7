...
export default function ProphetWelcome({ userName, onDismiss }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white border-2 border-[#7a1f30]/40 rounded-2xl max-w-2xl w-full p-8 shadow-2xl relative">
        <button onClick={handleDismiss} className="absolute top-4 right-4 text-slate-500 hover:text-slate-900">
          <X className="w-6 h-6" />
        </button>

        {/* Prophet Image */}
        <div className="flex flex-col items-center mb-6">
          <div className="w-32 h-32 rounded-full overflow-hidden border-4 mb-4" style={{borderColor: '#7a1f30'}}>
            <img
          ...
          <h2 className="text-2xl font-bold mb-1" style={{color: '#7a1f30'}}>Welcome, {userName}</h2>
          <p className="text-slate-600 text-sm">A Personal Greeting from Prophet Gad</p>
        </div>
        ...
        {!audioError && (
          <div className="flex items-center justify-center gap-3 mb-5">
            {isLoading ? (
              <div className="flex items-center gap-2 text-sm" style={{color: '#7a1f30'}}>
              ...
              <button
                ...
                style={{backgroundColor: 'rgba(122,31,48,0.1)', border: '1px solid rgba(122,31,48,0.5)', color: '#7a1f30'}}
              >
              ...
              <div className="flex gap-1 items-end h-5">
                {[...Array(5)].map((_, i) => (
                  <div key={i} className="w-1 rounded-full animate-pulse"
                    style={{ height: `${10 + i * 3}px`, animationDelay: `${i * 0.1}s`, backgroundColor: '#7a1f30' }} />
                ))}
              </div>
            )}
          </div>
        ...
        {/* Written Message */}
        <div className="text-slate-700 text-sm leading-relaxed space-y-3 mb-6">
          <p className="font-semibold" style={{color: '#7a1f30'}}>Welcome, {userName},</p>
          <p>
          ...
          <p className="font-semibold" style={{color: '#7a1f30'}}>
            Be faithful. Be watchful. Stay the course. The door of mercy is still open — but time is short.
          </p>
          <p className="text-slate-600 italic">Walk in the light. — Prophet Gad</p>
          <p className="text-center text-slate-500 text-xs italic mt-2 border-t border-slate-200 pt-3">
            "Blessed are your eyes for they see, and your ears for they hear." — Matthew 13:16
          </p>
        </div>

        <Button onClick={handleDismiss} variant="outline" className="w-full" style={{borderColor: 'rgba(122,31,48,0.5)', color: '#7a1f30'}}>
